...
module.exports = {
  ...
  run: async (sock, remitente, msg, obtenerMencionado) => {
    try {
      ...
      // MENSAJE ÚNICO Y ESTÉTICO: Emojis alineados a la izquierda y sin títulos molestos
      ...
      if (suerte === 3) {
        // === ¡PUM! LE TOCA LA BALA (CASTIGO DIVERTIDO DE ROL SIN SACAR A NADIE) ===
        textoRuleta += `💥 *¡💥 PUMMMM! 💥*\n\n`;
        ...
        await sock.sendMessage(remitente, { text: textoRuleta, mentions: [quienEnvia] }, { quoted: msg });
      } else {
        // === BANCO DE 5 FRASES DE SUPERVIVENCIA ALEATORIAS CONSERVADAS ===
      ...
      console.error('❌ Error crítico en el comando de ruletarusa.js:', error);
    }
  }
};

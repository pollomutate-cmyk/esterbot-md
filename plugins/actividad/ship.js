...
module.exports = {
  // CONFIGURADO: Quitamos la barra diagonal para compatibilidad total con tu index.js
  comando: ['ship'], 
  run: async (sock, remitente, msg, obtenerMencionado) => {
    try {
      ...
      // Descargamos los metadatos en vivo del chat para etiquetar el grupo de forma dinámica
      const metadatosGrupo = await sock.groupMetadata(remitente);
      const nombreGrupoActual = metadatosGrupo.subject || "COMUNIDAD";
      ...
      // === SECCIÓN CONSERVADA: Rangos de desamor y amor cruel intactos ===
      ...
      // Limpieza de firmas compatible con formatos híbridos @lid
      let num1 = String(usuario1).split('@')[0];
      let num2 = String(usuario2).split('@')[0];

      // CORRECCIÓN SOLICITADA: Título adaptativo que menciona el nombre del grupo actual en mayúsculas
      let textoShip = `💞 ═══ ${nombreGrupoActual.toUpperCase()} 𝐒𝐇𝐈𝐏 ═══ 💞\n\n`;
      textoShip += `💘 *Pareja:* @${num1}  x  @${num2}\n`;
      textoShip += `📊 *Porcentaje de Amor:* ${porcentajeAmor}% ${emojiAmor}\n\n`;
      textoShip += `📝 *Predicción:* _${comentario}_`;

      // Inyectamos tanto a los dos usuarios como al JID del grupo en la lista de menciones de WhatsApp
      await sock.sendMessage(remitente, { 
        text: textoShip, 
        mentions: [usuario1, usuario2, remitente] 
      }, { quoted: msg });

    } catch (error) {
      console.error('❌ Error crítico en el comando ship.js dinámico:', error);
    }
  }
};
